...
namespace LOS
{
    ...
    public class LOSFullScreenLight : LOSLightBase
    {
        ...
        {
            return _losCamera.halfViewboxSize.x;
        }
        ...
        protected override void ForwardDraw()
        {
            ...
            foreach (Vector3 corner in _losCamera.viewboxCorners)
            {
        ...
        protected override void InvertDraw()
        {
            ...
            // Add the four viewbox corners.
            for (int i = 0, n = _losCamera.viewboxCorners.Count; i < n; i++)
            {
                invertAngledMeshVertices.Add(_losCamera.viewboxCorners[i] - position);
            }
            ...
            for (float degree = _startAngle; degree < _endAngle; degree += degreeStep)
            {
                ...
                if (LOSRaycast(direction, out hit, _raycastDistance) && CheckRaycastHit(hit, 0))
                {
                    // Hit a collider.
                    Vector3 hitPoint = hit.point;
...
//							AddNewTriangle(invertAngledTriangles, previousClosePointIndex, closePointIndex, previousFarPointIndex);
                    ...
                    else
                    {
                        ...
                        else if (previousHitGo != hitGo)
                        {
                            ...
                            if (SMath.GetDegreeBetweenIndexVector(previousCloseToPreviousFar.normalized, previousClosePointToFar.normalized) < 0)
                            {
                                // left
